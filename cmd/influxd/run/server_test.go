...
func TestServer_Query_DefaultDBAndRP(t *testing.T) {
	...
	test.addQueries([]*Query{
		...
		&Query{
			...
			exp:     `{"results":[{"statement_id":0,"series":[{"columns":["name","duration","shardGroupDuration","replicaN","default"],"values":[["autogen","0","168h0m0s",1,false],["rp0","0","168h0m0s",1,true]]}]}]}`,
		},
		&Query{
			name:    "default rp",
...
func TestServer_Query_With_EmptyTags(t *testing.T) {
	...
	test.addQueries([]*Query{
		...
		&Query{
			...
			params:  url.Values{"db": []string{"db0"}},
			command: `select value from cpu where host =~ /.*/`,
			exp:     `{"results":[{"statement_id":0,"series":[{"name":"cpu","columns":["time","value"],"values":[["2009-11-10T23:00:02Z",1],["2009-11-10T23:00:03Z",2]]}]}]}`,
		},
		&Query{
